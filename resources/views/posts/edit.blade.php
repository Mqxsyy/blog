            ...
            <div class="card-body">
                <form method="POST" action="{{ route('posts.update', ['post' => $post]) }}">
                    ...
                    <label class="form-control w-full">
                        ...
                        <textarea rows="12" name="body" placeholder="Write something cool..."
                            class="textarea textarea-bordered @error('body') textarea-error @enderror w-full">{{ old('body') ?? $post->body }}</textarea>
                        <div class="label">
                            @error('body')
                                <span class="label-text-alt text-error">{{ $message }}</span>
                            @enderror
